...
;; Add a this test case? (intersection '(1 2 2 3 4) '(2 4 5))

;; Type signature: (intersection list list) -> set

;;(make-set (union LA LB))
...
(define (intersection A B)
  (cond [(null? A) '()]
        [(element? (car A) B)(cons (car A)(intersection (cdr A) B))]
        [else (make-set (intersection (cdr A) B))]))
...
(define (subset? SA SB)
  (cond ((null? SA) #t)
    ((element? (car SA) SB) (subset? (cdr SA) SB))
    (else #f))
  )
...
(define (set-difference LA LB)
  (if (null? LA) '()
      (if (element? (car LA) LB)
          (make-set (set-difference (cdr LA) LB))
          (make-set (cons (car LA) (set-difference (cdr LA) LB)))))
  )
...
(define (sym-diff A B)
  (cond [(null? A) B]
        [(element? (car A) B) (sym-diff (cdr A) (remove (car A) B))]
        [else (make-set(cons (car A) (sym-diff (cdr A) B)))]))
...
(define (remove e S)
  (cond [(null? S) '()]
        [(element? e S) (remove e (cdr S))]
        [else (cons (car S) (remove e (cdr S)))])
  )
|#
(define (remove e S)
  (if (null? S) '()
     (if (equal? (car S) e) (cdr S)
