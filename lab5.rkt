...
(define (distinct-elems? tuple)
    ;;(cond [equal? 0 (snd (tuple)) #t])
    (cond
      [(or (null? tuple) (null? (cdr tuple))) #t]  ;;less than 2 elements
      [(member (car tuple) (cdr tuple)) #f]      ;;duplicate elements
      [else (distinct-elems? (cdr tuple))])
    )
...
(define (repeat k lst)
   (cond
     [(eqv? k 0) `()]
     [(append (list lst) (repeat (- k 1) lst))] ;;appends to a new created list and recurses
     )
  )
...
(define (pair-up x lst)
  #|(cond
    [(zero? x) lst]
    [(rcons x lst (repeat x lst (cadr lst)))]
    )|#
  (cond
    [(null? lst) '()]
    [(list? (car lst)) (cons (rcons x (car lst)) (pair-up x (cdr lst)))]
    [else (cons (rcons x (list (car lst))) (pair-up x (cdr lst)))])
  )
...
(define (cartesian-product list-of-lists)
  (if (element? `() list-of-lists) `()
      (if (equal? (length list-of-lists) 2)
          (cartesian-product2 (car list-of-lists) (car (cdr list-of-lists)))
          (cartesian-product2 (car list-of-lists) (cartesian-product (cdr list-of-lists))) 
          )
      )
  )
