...
;; (pred '((())) )(pred '() )(tst1 pred 1)(tst1 pred 38) (pti (pred (pred (pred (itp 14)))))
(define (pred p)
  (if (pz? p) (eopl:error "pred error: zero has no predecessor!") (car p))
  )
...
;;(p+ '((())) '(()) ) (tst2 p+ 0 0) (tst2 p+ 5 4) (tst2 p+ 1 6)(tst2 p+ 0 10)
...
;; (p- '((((())))) '((())) )(tst2 p- 12 12) (tst2 p- 6 4)(tst2 p- 4 6)(tst2 p- 30 19)(tst2 p- 10 0)
...
;;
;; Type signature: (p* peano peano) -> peano
;; (p* '(((()))) '((())) ) (tst2 p* 0 0)(tst2 p* 3 0)(tst2 p* 4 3)(tst2 p* 1 7)(tst2 p* 17 5)
(define (p* a b)
  (cond ((pz? b) z)
        (else (p+ a (p* a (pred b))))
    )
...
;; (p^ '((())) '(((()))) )(tst2 p^ 0 4)(tst2 p^ 5 0)(tst2 p^ 4 2)(tst2 p^ 3 3) (tst2 p^ 2 7)
(define (p^ a b)
  (cond [(pz? b) (succ z)]
        [else (p* a (p^ a (pred b)))]
    )
...
;;  (p= '((())) '(()) ) (p= '(((()))) '(((()))) )(tst2 p= 3 16)(tst2 p= 5 5)  (tst2 p= 0 0)
(define (p= a b)
...
;; (p> '((())) '(()) )(tst2 p> 4 6)(tst2 p> 3 3)(tst2 p> 7 2)
(define (p> a b)
 (cond [(pz? a) #f]
       [(pz? b) #t]
       [else (p> (pred a) (pred b))]
...
;;  (p% '(((((()))))) '(((()))) )(tst2 p% 3 5)(tst2 p% 0 6)(tst2 p% 30 15)(tst2 p% 8 1)(tst2 p% 124 17)(tst2 p% 76 9)
(define (p% a b)
 (cond [(pz? b) (eopl:error "modulo error: mode is zero!")]
       [(or (p> b a) (pz? a)) a]
       [else (p% (p- a b) b)]
    )
  )
...
;; (p-even? '((((())))) ) (tst1 p-even? 0) (tst1 p-even? 1)(tst1 p-even? 25)(tst1 p-even? 76)
(define (p-even? p)
 (pz? (p% p (succ (succ z))))
  )
...
;; (p-prime? '(((((((()))))))) )(tst1 p-prime? 0)(tst1 p-prime? 2) (tst1 p-prime? 5) (tst1 p-prime? 9)(tst1 p-prime? 35)(tst1 p-prime? 37)

(define (helper x y)
  (cond [(p= x (succ z)) #t]
        [(pz? (p% y x)) #f]
        [else (helper (pred x) y)]))

(define (p-prime? p)
  (cond [(or (p= p (succ z)) (pz? p)) #f]
        [(p= p (succ (succ z))) #t]
        [else (helper (pred p) p)]
    )
  )
